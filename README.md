...
-	Run ```make``` in ```modules/tuningMod/``` and insert the module into the kernel
	* Run ```insmod sudo insmod tuningModule.ko```
	* Find the line ```tuningModule has been loaded``` in /var/log/kern.log and there will be a 
		number next to it.  This is the MAJOR number. ```e.g. tuningModule has been loaded 508```
